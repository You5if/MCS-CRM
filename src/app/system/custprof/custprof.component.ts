...
import { CustProfEntryComponent } from './custprof-entry/custprof-entry.component';
import { CustProfModel } from './custprof.model';
import { RightModel } from 'src/app/components/security/auth/rights.model';
import { RouterModule, Routes } from '@angular/router';
import { PageSortComponent } from 'src/app/components/common/pageevents/page-sort/page-sort.component';
import { CustProfService } from './custprof.service';
...
@Component({
    ...
    templateUrl: './custprof.component.html',
    styleUrls: ['./custprof.component.scss']
  })

export class CustProfComponent implements OnInit {

    displayedColumns: string[] =
        ['custNo','fullName', 'custType', 'clientStatus'];
    ...
    menuId: number;
    clickedRows = new Set<CustProfModel>();
    selection = new SelectionModel<CustProfModel>(true, []);custType: string;
;
  ...
  workShimmer:boolean;
    ...
    constructor(
        ...
        private custprofservice: CustProfService
      ) {
        this.pTableName = 'CustProf';
        this.pScreenId = 109;
        this.pTableId = 109;
  ...
  refreshMe() {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.submit = "Submit"
      this.cancel = "Cancel"
      this.headerToShow = [this.custNo, this.fullName,this.custType, this.clientStatus]
  ...
  onView = (id: number) => {
    ...
    this.custprofservice.getCustProfEntry(id).subscribe((result: CustProfModel) => {
      this._ui.loadingStateChanged.next(false);
      result.entryMode = 'V';
  ...
  onId(id: number, row:CustProfModel) {
  ...
  openEntry  (result: CustProfModel) {
    if (result === undefined) {
      const dialogRef = this.dialog.open(CustProfEntryComponent, {
        disableClose: true,
        data: {}
      });
