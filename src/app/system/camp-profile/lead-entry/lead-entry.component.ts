...
export class CProfileEntryComponent implements OnInit {

  dummy = {
    campCode: 'CAMP00'+(this.dummyService.campProfile.length + 1).toString(),
    campName: '',
    campObjective: '',
...
campHost = [
  ...
  onSubmit() {
    ...
    this.campHost.forEach((index) => {
      if(index.id === this.dummy.hostId){
        this.dummy.campHost = index.name
      }
    })
    this.dummyService.campProfile.push(this.dummy)
