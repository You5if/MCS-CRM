...
export class AttendanceuploadComponent implements OnInit {
  ...
  public uploadFinished = (event:any) => {
    // this is event being called when file gets uploaded
  ...
  onSubmit() {
    this.http
      .post(this._globals.baseAPIUrl + "HRMSAttendance/DDo", {})
      .subscribe((res: any) => {
        if (res.errorNo === 0) {
          this._msg.showInfoSuccess("Success", "Files uploaded");
        } else {
          this._msg.showInfoError("Error", res.errorMessage);
        }
      });
  }
  ...
  onSubmitFile(): void {
    const data = { fileName: this.lFiles[0].fullPath };
    this.http
      .post(this._globals.baseAPIUrl + "HRMSAttendance/upload", data)
      .subscribe((res: any) => {
        if (res.errorNo === 0) {
          this._msg.showInfoSuccess("Success", "File submitted");
